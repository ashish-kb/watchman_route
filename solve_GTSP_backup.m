...
%     title('Given Completed Graph with duplicate nodes added and orphan nodes removed');
    ...
    IN_tab_Interedge = IN_transform_Interedge(G_comp);
...
%     plot(G_gadget, 'EdgeLabel', G_gadget.Edges.Weight);
    ...
    init_Clus_vec = cell2mat(cellfun(@(x) str2double(strsplit(x((regexp(x,'-','start')+1):end),'-')) , G_comp.Edges.EndNodes(:,1),'uni',0));
    end_Clus_vec = cell2mat(cellfun(@(x) str2double(strsplit(x((regexp(x,'-','start')+1):end),'-')) , G_comp.Edges.EndNodes(:,2),'uni',0));
    
    init_node_vec = cellfun(@(x) str2double(strsplit(x(2:(regexp(x,'-','start')-1)),'-')) , G_comp.Edges.EndNodes(:,1),'uni',0);
    end_node_vec  = cellfun(@(x) str2double(strsplit(x(2:(regexp(x,'-','start')-1)),'-')) , G_comp.Edges.EndNodes(:,2),'uni',0);
