...
G_comp.Nodes.Cluster{1} = [3, 1, 4];
G_comp.Nodes.Cluster{2} = [5, 2, 1];
G_comp.Nodes.Cluster{3} = [3, 5, 6];
G_comp.Nodes.Cluster{4} = [5, 3, 4, 6];
...
for k = 1:r_nodesize
    ...
    if( (prenode_tab.Cluster{k}~=0)) % for k ==1 already created above and second condition along with  prenode_tab.Cluster(match_clus) = {0}; below ensures code is not repeated for already created nodes
