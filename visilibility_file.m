...
% [guards_x, guards_y] = ginputc('ShowPoints', true);
% guards = [guards_x, guards_y];

guards = read_vertices_from_file('./ginput.guards'); 
guards = guards{1}; % converts cell to mat

%Plot Guards

plot3(guards(:,1), guards(:,2), 0.4*ones(1,length(guards(:,2))), 'o','Markersize',9,'MarkerEdgeColor','y','MarkerFaceColor','k');
